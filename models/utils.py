...
def pad(x): return torch.nn.functional.pad(x, (12, 12, 7, 7))
...
class MyDistributedIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        num_couples = int(len(all_pairs) * zeros_pop_ratio)
        couple_indices = random.sample(range(len(all_pairs)), num_couples)
        for index in couple_indices:
            folder, k_map, costhab_map, rep, year = all_pairs[index]
            all_pairs.insert(index + 1, (folder, k_map, costhab_map, rep, -10))
            all_pairs[index] = (folder, k_map, costhab_map, rep, -10)
        ...
        for folder, k_map, costhab_map, rep, year in all_pairs:
            if year == -10:
                map_now = torch.zeros_like(k_map)
                map_future = torch.zeros_like(k_map)
                delta_map = torch.zeros_like(k_map)
            ...
            # Prepare input tensors
            cond_pop = pad(map_now).repeat(1, 3, 1, 1)
            cond_k = pad(k_map).repeat(1, 3, 1, 1)
            cond_costhab = pad(costhab_map).repeat(1, 3, 1, 1)

            pred = pad(delta_map).repeat(1, 3, 1, 1)
            # pred = pad(map_future).repeat(1, 3, 1, 1)

            # cond_pop = map_to_tensor(pad(map_now)).type(self.__data_type)
            # cond_k = map_to_tensor(pad(k_map)).type(self.__data_type)
            # cond_costhab = map_to_tensor(pad(costhab_map)).type(self.__data_type)
...
class MyDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        dataset = MyDistributedIterableDataset(
            ...
            reps=range(0, self.train_threshold),
            years=self.years,
            # vae=self.vae
    ...
    def val_dataloader(self):
        dataset = MyDistributedIterableDataset(
            ...
            reps=range(self.train_threshold, self.validation_threshold),
            years=self.years,
            data_type=self.data_type
            # vae=self.vae
        )
    ...
    def test_dataloader(self):
        dataset = MyDistributedIterableDataset(
            args=self.args,
            folders=self.validation_folders,
            reps= range(self.validation_threshold, 20),
            years=self.years,
            data_type=self.data_type
            # vae=self.vae
        )
...
class EMACallback(Callback):
    ...
    def on_train_start(self, trainer, pl_module):
        # Initialize EMA state from model weights
        self.ema_state = {
            k: v.detach().clone()
    ...
    def on_after_backward(self, trainer, pl_module):
        # Update EMA state after each gradient step
    ...
    def apply_ema_weights(self, pl_module):
        """Copy EMA weights into the model (for validation/sampling)."""
        pl_module.load_state_dict(self.ema_state, strict=False)

    def restore_original_weights(self, pl_module, original_state):
        """Restore original model weights after using EMA."""
        pl_module.load_state_dict(original_state, strict=False)